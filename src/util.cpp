...
const char * const BITCOIN_CONF_FILENAME = "arepa.conf";
const char * const BITCOIN_PID_FILENAME = "arepad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "arepa" is a composite category enabling all Arepa-related debug output
            if(ptrCategory->count(string("arepa"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArepaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArepaCore
    // Mac: ~/Library/Application Support/ArepaCore
    // Unix: ~/.arepacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArepaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArepaCore";
#else
    // Unix
    return pathRet / ".arepacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty arepa.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override arepa.conf
